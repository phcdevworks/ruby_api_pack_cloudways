...
RSpec.describe RubyApiPackCloudways::Connection::CwConnect do
  ...
  before do
    allow(RubyApiPackCloudways::Connection::CwToken).to receive(:new).and_return(instance_double(
                                                                                   RubyApiPackCloudways::Connection::CwToken, cw_api_token: 'fake_token'
                                                                                 ))
  end
  ...
  describe '#cloudways_api_connection' do
    it 'creates a HTTParty connection' do
      ...
      expect(HTTParty).to have_received(:get).with('https://api.cloudways.com/api/v1/some_path',
